        ...
        <BlueCard className="flex flex-col gap-2.5">
          ...
          {services.map((s) => (
            <Link key={s.serviceId} href={`/service-track?trackingId=${s.serviceId}`} className="flex items-center gap-3 p-3 rounded-[10px] bg-[#f5f7fb]">
              <span className={`size-11 rounded-[10px] flex items-center justify-center shrink-0 ${s.status === "completed" ? "bg-[#e3f5ea] text-[#1a7f45]" : "bg-[#e6f3ff] text-[#1f6fd0]"}`}>{s.type === "install" ? <Home size={20} /> : <Wrench size={20} />}</span>
